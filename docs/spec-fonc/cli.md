...
| `systemlens flows [list] [--root DIR] [--json] [--publishes-to-topic]` | Lists persisted potential code flows from an entry point to a source-evidenced external effect, within one method or across method calls resolved by the selected method-call engine. Each summary exposes `input_topic` and `output_topics`; `--publishes-to-topic` keeps only flows with at least one Kafka publication. Each flow reports whether its endpoint evidence is `complete` or `partial` relative to the persisted topology snapshot. |
