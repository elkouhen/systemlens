...
| `systemlens flows [list] [--root DIR] [--json] [--publishes-to-topic]` | Lists persisted potential code flows from an entry point to a source-evidenced external effect, within one method or across method calls resolved by the selected method-call engine. Each summary exposes the module, input flow and Java type, output flow and Java type, plus the compatibility fields `input_topic` and `output_topics`. Unknown types remain `null`; `--publishes-to-topic` keeps only flows with at least one Kafka publication. Each flow reports whether its endpoint evidence is `complete` or `partial` relative to the persisted topology snapshot. |
